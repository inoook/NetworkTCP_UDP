...
//{
  ...
  public class OscMessage
  {
   ...
   public ArrayList Values;
	...
	{
		if( !checkDataLength(i) ){ return 0; }
		return (int)( Values[i] );
	}
	public float getArgAsFloat(int i)
	{
		if( !checkDataLength(i) ){ return 0; }
		return (float)( Values[i] );
	}
	public string getArgAsString(int i)
	{
		if( !checkDataLength(i) ){ return ""; }
		return ( Values[i] ).ToString();
	}
	private bool checkDataLength(int i)
	{
		if(i < Values.Count ){
			return true;
		}
		Debug.LogError("DataLengthError: "+i + " / "+Values.Count);
		return false;
	}
  }
...
//      ReadThread.Start();
	...
	public static string OscMessageValueToString(OscMessage message)
    {
      StringBuilder s = new StringBuilder();
      for( int i = 0; i < message.Values.Count; i++ )
      {
		object o = message.Values[i];
		if(i > 0){
        	s.Append(",");
		}
        s.Append(o.ToString());
